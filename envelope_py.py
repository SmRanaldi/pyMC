...
def adaptive_envelope(data_in):
    data_c = np.array(data_in, dtype=DTYPE)
    return _adaptive_envelope_main(data_c)

def _adaptive_envelope_main(data_in):

    data = _preWhiten(data_in)

    # --- Initialization
    m = np.ones(data.shape, dtype=DTYPE)*L_INIT
    w = np.zeros(data.shape, dtype=DTYPE)
    d1 = np.zeros(data.shape, dtype=DTYPE)
    d2 = np.zeros(data.shape, dtype=DTYPE)
    nsamples = m.shape[0]
    ...
    for i in range(nsamples):
        m[i] = _single_sample(data, m[i], w[i], d1[i], d2[i], i, CHI)
    w=np.asarray([_update_w(data, m[i], i) for i in range(nsamples)])
    return w,m
...
def adaptive_envelope_par(data_in):
    ...
    # --- Initialization
    m = np.ones(data.shape).flatten()*L_INIT
    w = np.zeros(data.shape).flatten()
    d1 = np.zeros(data.shape).flatten()
    d2 = np.zeros(data.shape).flatten()
    nsamples = m.shape[0]
